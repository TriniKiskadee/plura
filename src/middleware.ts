import {auth, clerkMiddleware, ClerkMiddlewareAuth, createRouteMatcher} from "@clerk/nextjs/server";
...
const afterAuth = async (auth: ClerkMiddlewareAuth, req: NextRequest) => {
    ...
    const pathWithSearchParams = `${url.pathname}${searchParams.length > 0 
        ? `?${searchParams}`
        : ''
    }`;
    ...
    // if requesting sign-in/sign-up
    /*TODO: Investigate if `/agency/${url.pathname} will work`*/
