...
const FormSchema = z.object({
    ...
    companyPhone: z.string().min(1)
        .regex(/^\+?\d{1,4}\s?\(?\d{1,5}\)?[\s.-]?\d{1,5}[\s.-]?\d{1,5}[\s.-]?\d{1,5}$/, {
            message: 'Invalid phone number format. Expected formats: +1 (868) 123-4567, 123-456-7890, etc.'
        }),
...
const AgencyDetails = ({data}: AgencyDetailsProps) => {
        ...
        try {
            ...
            if(!data?.id) {
                ...
                toast({
                    ...
                    description: 'Successfully created your agency!'
                })
            ...
            console.log(`🔴 agency-details handleSubmit Error: ${error}`)
            toast({
                title: 'Oppse! Something went wrong...',
                description: 'Your agency was not created, please try again later',
                variant: 'destructive'
            })
        }
    }
    ...
    const handleDeleteAgency = async () => {
        ...
        /*TODO: discontinue the stripe subscription*/

        const response = await deleteAgency(data.id)
        if (!response) {
            toast({
                title: 'Oppse! Something went wrong...',
                description: 'Your agency was not deleted, please try again later.',
                variant: 'destructive'
            })
        } else {
            toast({
                title: 'Agency Deleted',
                description: 'Successfully deleted your agency and all subaccounts!',
            })
        }
        router.refresh()
        ...
        <AlertDialog>
            <Card className={'w-full'}>
                <CardHeader>
                    ...
                    <CardDescription>
                        Let's create an agency for you business.
                    ...
                    <Form {...form}>
                        ...
                        >
                            ...
                            <div className={'flex md:flex-row gap-4'}>
                                ...
                                />
                                {/*TODO: Hydration error due to PW manager*/}
                                <FormField
                                    control={form.control}
                                    name={'companyEmail'}
                                    render={({field}) => (
                    ...
                    {data?.id && (
                        <div
                            ...
                            <div className={'text-muted-foreground'}>
                                ...
                                all
                                data related to your sub accounts. Sub accounts will no longer have access to
                                funnels,
                                contacts etc.
                            </div>
                            <AlertDialogTrigger
                                disabled={isLoading || deletingAgency}
                                className={'text-red-600 p-2 text-center mt-2 rounded-md border border-red-600 hover:bg-red-600 hover:text-white whitespace-nowrap'}
                            >
                                {deletingAgency
                                    ? 'Deleting...'
                                    : 'Delete Agency'
